...
class Messages:
    ...
    async def send_file(self, chat_id, file_id, file_path, caption):  # отправка файла: .pdf, .png, .docx, .jpg и пр.
        try:
    ...
    async def send_album(self, chat_id, files_id: list, files_path: list, caption):  # отправка альбома с .jpg | .png
        album_builder = []
    ...
    async def message_day_1(self, chat_id):
        await self.send_text(chat_id=chat_id, text='Это первая цепочка. В функцию можно добавить сколько угодно '
                                                   'сообщений, в том числе с фото, ссылками, файлами, видео и пр.')
        await self.send_file(chat_id=chat_id,
        ...
        await self.send_album(chat_id=chat_id,
                              files_id=[
